/*
	Calculate solution for 3 variable, 3 equation system using gauss-jacobi method
*/
...
void gauss(float m[line_max + 1][col_max + 1], float x[line_max+1], int ite){
	...
	for(unsigned int i = 0; i < ite; i++){
		z1 = (m[0][col_max] - (m[0][1]*x2 + m[0][2]*x3))/m[0][0];
		z2 = (m[1][col_max] - (m[1][0]*x1 + m[1][2]*x3))/m[1][1];
		z3 = (m[2][col_max] - (m[2][0]*x1 + m[2][1]*x2))/m[2][2];
